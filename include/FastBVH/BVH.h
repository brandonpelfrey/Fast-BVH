...
struct Node final {
  ...
  //! If non-zero, this is the offset to the sub node
  //! within the BVH that this node came from.
  uint32_t right_offset;
  //! Indicates if this node is a leaf node.
  //! \return True if this node is a leaf node, false otherwise.
  inline constexpr bool isLeaf() const noexcept {
    return right_offset == 0;
  }
};
...
public:
  ...
  BVH(NodeArray<Float>&& n, const ConstIterable<Primitive>& p)
    : nodes(std::move(n)), primitives(p) {}
  ...
  inline auto getNodes() const noexcept {
    return ConstIterable<Node<Float>>(nodes.data(), nodes.size());
  }
  //! Accesses an iterable container to the primitives in the BVH.
  //! \return An iterable container of the primitive array.
  inline auto getPrimitives() const noexcept {
    return primitives;
  }
};
