...
int main() {
  ...
  //auto r0 = FastBVH::Benchmark::bench<0>(faces, reader.GetAttrib());

  std::printf("Running benchmark for strategy 1\n");
  auto r1 = FastBVH::Benchmark::bench<1>(faces, reader.GetAttrib());

  std::vector<FastBVH::Benchmark::StrategyResults> results = { r1 };
