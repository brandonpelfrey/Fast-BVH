...
void runBenchmark() {
  ...
  std::printf("  Build completed in   : %06.04f seconds\n", build_time);

  stopwatch.reset();

  codes.sort();

  auto sort_time = stopwatch.read();

  std::printf("  Sorting completed in : %06.04f seconds\n", sort_time);

  std::printf("  Total time           : %06.04f seconds\n", build_time + sort_time);
}
